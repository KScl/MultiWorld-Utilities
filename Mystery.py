...
def roll_settings(weights: dict, plando_options: typing.Set[str] = frozenset(("bosses"))):
    ...
    glitches_map = {
        ...
        'overworld_glitches': None, # not supported in doors branch
        'no_logic': 'nologic',
    }
    ...
    # doors branch does not support owg, filter out to noglitches
    if ret.logic is None:
        logging.warning(f'"{glitches_required}" logic is not supported, defaulting to "no_glitches"')
        ret.logic = 'noglitches'
    ...
    if 'rom' in weights:
        ...
        ret.msuresume = get_choice('msuresume', romweights, True)
